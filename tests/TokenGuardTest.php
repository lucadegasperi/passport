...
class TokenGuardTest extends TestCase
{
    ...
    public function test_user_can_be_pulled_via_bearer_token()
    {
        ...
        $userProviderResolver = m::mock(UserProviderResolver::class);
        $tokens = m::mock(TokenRepository::class);
        $clients = m::mock(ClientRepository::class);
        $encrypter = m::mock(Encrypter::class);

        $guard = new TokenGuard($resourceServer, $userProviderResolver, $tokens, $clients, $encrypter);

        $request = Request::create('/');
        $request->headers->set('Authorization', 'Bearer token');

        $resourceServer->shouldReceive('validateAuthenticatedRequest')->andReturn($psr = m::mock());
        $psr->shouldReceive('getAttribute')->with('oauth_user_id')->andReturn('users-1');
        $psr->shouldReceive('getAttribute')->with('oauth_client_id')->andReturn(1);
        $psr->shouldReceive('getAttribute')->with('oauth_access_token_id')->andReturn('token');
        $userProviderResolver->shouldReceive('resolve')->with('users-1')->andReturn($userProvider = m::mock(UserProvider::class));
        $userProviderResolver->shouldReceive('getUsername')->with('users-1')->andReturn(1);
    ...
    public function test_default_provider_is_used_to_retreive_users_from_cookies_with_csrf_token_header()
    {
        $resourceServer = m::mock(ResourceServer::class);
        $userProviderResolver = m::mock(UserProviderResolver::class);
        $tokens = m::mock(TokenRepository::class);
        ...
        $userProviderResolver->shouldReceive('resolve')->with('1')->andReturn($userProvider = m::mock(UserProvider::class));
        $userProviderResolver->shouldReceive('getUsername')->with('1')->andReturn(1);
        $userProvider->shouldReceive('retrieveById')->with(1)->andReturn($expectedUser = new TokenGuardTestUser);
    ...
    public function test_cookie_xsrf_is_verified_against_csrf_token_header()
    {
        ...
        $userProviderResolver->shouldReceive('resolve')->never();
    ...
    public function test_expired_cookies_may_not_be_used()
    {
        ...
        $userProviderResolver->shouldReceive('retrieveById')->never();
